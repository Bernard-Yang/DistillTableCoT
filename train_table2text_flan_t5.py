...
import sys
# from lightning_base_flan import BaseTransformer, add_generic_args, generic_train, get_linear_schedule_with_warmup
...
class SummarizationTrainer(BaseTransformer):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self._step(batch)
        # output = {"val_loss": loss, "preds": preds, "target": target}
    ...
    # def validation_epoch_end(self):
        ...
        if "preds" in outputs[0]:
            ...
            avg_loss = torch.stack([x["val_loss"] for x in outputs]).mean()
            # avg_loss = torch.stack([x for x in outputs if x == "val_loss"]).mean()
