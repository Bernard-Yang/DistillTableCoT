...
"""
Evaluate the fidelity of CONTLOG dataset with TAPEX.
...
class DataTrainingArguments:
    ...
    test_name: Optional[str] = field(default=None, metadata={"help": "A .txt file containing the model outputs to evaluate."})
    data_dir: Optional[str] = field(default='data/contlog',
...
def main():
    ...
    def preprocess_logic2text_function(examples):
        # Tokenize the texts
        def _construct_table(table):
            table_header, table_cont = table
            # if str(table_header).strip() != "['Gong et\xa0al.  [BOLD] ( 2018  [BOLD] )', 'topic_science', 'topic_science', 'topic_science', 'topic_science', 'topic_science', '[BOLD] P .758', '[BOLD] P ±.012', '[BOLD] R .885', '[BOLD] R ±.071', '[BOLD] F .818', '[BOLD] F ±.028']":
            # print('len(table_header)', len(table_header))

            # for i in [len(i) for i in table_cont]:
            #     if i != len(table_header):
            #         print('table_header', table_header)
            # print('table_header', table_header)
            # print('table_cont', table_cont)
        ...
        tables = list(map(_construct_table, zip(examples['table_column_names'], examples['table_content_values'])))
    ...
    def prepare_single_data(args, output_dir, split, test_name):
        with open(os.path.join(args.data_dir, f'{split}.json'), 'r', encoding='utf-8') as fin, \
                open(test_name) as fsent, \
                open(os.path.join(output_dir, args.affix, f'for_{split}.json'), 'w', encoding='utf-8') as fout:
            data = json.load(fin)
            sents = fsent.readlines()
            for i, (d, s) in enumerate(zip(data, sents)):
                d_new = {}
                d_new['id'] = i + 1
                d_new['table_column_names'] = d['table_column_names']
                d_new['table_content_values'] = d['table_content_values']
                d_new['sent'] = s.rstrip()
                d_new['label'] = 1
    ...
    if data_args.dataset_name is not None:
        ...
        if training_args.do_predict and data_args.test_name is not None:
            ...
            # the test file is reformalized to a json file
    ...
    with training_args.main_process_first(desc="dataset map pre-processing"):
        raw_datasets = raw_datasets.map(
            preprocess_logic2text_function,
            batched=True,
